...
class ronnAnimation{
  ...
  public:
    ...
    void printBigChar(byte c, int x, int y){ //char,X,Y
       byte l = pgm_read_byte_near(font_BIG + (c * 33))+1;  //get pixel width of character font_BIG
      for (int a=0;a<16;a++){
    ...
    void printBigText(char* string, int x, int y){ //string,X --> 16 pixel height, y always 0
      while (*string){
        byte c = *string-32;
        printBigChar(c,x,y);
